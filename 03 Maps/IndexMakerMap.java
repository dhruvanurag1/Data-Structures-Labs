...
class DocumentIndex extends TreeMap<String, TreeSet<Integer>>
{
   ...
   */
   public void addAllWords(String str, int lineNum) 
   {
      ...
      for(int i = 0; i < array.length; i++) {
         if(array[i].length() >= 1) {
            addWord(array[i].toUpperCase(), lineNum);
         }
      }
   }
   ...
   */
   ...
   public String toString() {
      String answer = "";
      for(String s: IndexMakerMap.index.keySet()) {
         answer += s + " " + convertTreeSet(IndexMakerMap.index.get(s)) + "\n";
      }
      return answer;
   }
   public String convertTreeSet(TreeSet<Integer> set) {
      String answer = "";
      for(Integer a : set) {
